		...
		<view class="content-td">
			<view v-if = "currentItem && currentItem.type =='text'">
				<view class="ps-edit-item">
					...
					<view class="bottom">
						<view class="colors">
							<view v-for="(c,index) in colorList" :key = "index" @click="modifyCurrentItem('color',c)" class="color-item" :style="{'background':c}"></view>
						</view>
					</view>
				</view>
				<view class="ps-edit-item">
					...
					<view class="bottom">
						  <slider :value="currentItem.detail.fontSize" min="12" max="88" @change="sliderChange" show-value step="5" />
					</view>
				</view>
				<view class="ps-edit-item">
					<view class="top">
					...
					<view class="bottom">
						<view class="set-image">
							...
							<view class="set-image-btn">
								<view class="iconfont icon-listing-content" @click="chooseImage"></view>
	...
	export default {
		...
		data() {
			return {
				...
				currentItem:null,
				colorList:['#ff1d20','#ffaa00','#ffff00','#55ff00','#0565ff','#55ffff','#8800ff'],
		...
		methods: {
			...
			sliderChange(e){
				...
				this.updateContentDetail({key:'fontSize',value})
			},
			chooseImage(){
				uni.chooseImage({
					count:1,
